...
export function createAudioEnvelope(): AudioEnvelope {
	...
	function tick(
		...
		const riseStep =
			1 - Math.exp(-(3 + attack * 14) * responseSpeed * safeDt);
		const fallStep =
			1 - Math.exp(-(0.8 + release * 16) * responseSpeed * safeDt);
		...
		const normalizedAmplitude = clamp(
			(smoothedAmplitude - adaptiveFloor) / usableRange,
			0,
			1
		);
		...
		const targetValue =
			cfg.min + (cfg.max - cfg.min) * clamp(excitedAmplitude, 0, 1);
