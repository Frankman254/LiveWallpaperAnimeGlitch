...
export function drawFilmNoise(
	...
	const particleCount = Math.floor(
		Math.min(5600, (width * height) / 700) * noiseIntensity
	);
	for (let i = 0; i < particleCount; i++) {
		...
		const alpha =
			(0.07 + seededRandom(seed + 9.1) * 0.28) * noiseIntensity * opacity;
	...
	if (seededRandom(scratchSeed) > 0.28) {
		...
		const scratchWidth =
			1 + Math.floor(seededRandom(scratchSeed + 5.7) * 2);
	...
	if (seededRandom(bandSeed + 1.3) > 0.42) {
		...
		const bandHeight = Math.max(
			2,
			height * (0.008 + seededRandom(bandSeed + 3.1) * 0.022)
		);
	...
	if (seededRandom(shadowBandSeed + 0.8) > 0.48) {
		...
		const bandHeight = Math.max(
			2,
			height * (0.006 + seededRandom(shadowBandSeed + 3.8) * 0.018)
		);
...
export function getScanlineAmount(
	mode: string,
	intensity: number,
	time: number,
	amplitude: number
	...
	if (mode === 'pulse') {
		return (
			intensity * (0.45 + 0.55 * (Math.sin(time * 0.0026) * 0.5 + 0.5))
		);
	}
	if (mode === 'burst') {
		return (
			intensity *
			(0.25 + seededRandom(Math.floor(time * 0.002) + 11.0) * 1.1)
		);
	}
	if (mode === 'beat') {
