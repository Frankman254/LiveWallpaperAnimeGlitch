...
export default function GlobalBackgroundView() {
  ...
  useEffect(() => {
    ...
    if (!canvas || !image || !store.globalBackgroundUrl || !store.globalBackgroundEnabled) return
    ...
    const frame = (time: number) => {
      ...
      const filterActive = state.filterTargets.includes('global-background')
      const brightness = state.globalBackgroundBrightness * (filterActive ? state.filterBrightness : 1)
      const contrast = state.globalBackgroundContrast * (filterActive ? state.filterContrast : 1)
      const saturation = state.globalBackgroundSaturation * (filterActive ? state.filterSaturation : 1)
      const blur = state.globalBackgroundBlur + (filterActive ? state.filterBlur : 0)
      const hue = state.globalBackgroundHueRotate + (filterActive ? state.filterHueRotate : 0)
      const opacity = state.globalBackgroundOpacity * (filterActive ? state.filterOpacity : 1)
      const rgbShiftPixels = filterActive ? state.rgbShift * Math.min(currentCanvas.width, currentCanvas.height) * 0.65 : 0
      const filmNoiseAmount = filterActive ? state.noiseIntensity : 0
      const audio = getAudioSnapshot()
      const scanlineAmount = filterActive
        ? getScanlineAmount(state.scanlineMode, state.scanlineIntensity, time, audio.amplitude)
        : 0
      ...
      const width = base.width * Math.max(0.01, state.globalBackgroundScale)
      const height = base.height * Math.max(0.01, state.globalBackgroundScale)
      const cx = currentCanvas.width / 2 + state.globalBackgroundPositionX * currentCanvas.width * 0.5
      const cy = currentCanvas.height / 2 - state.globalBackgroundPositionY * currentCanvas.height * 0.5
      ...
      if (filterActive) {
        ...
        drawFilmNoise(ctx, width, height, filmNoiseAmount, time, ctx.globalAlpha)
        drawScanlines(ctx, width, height, scanlineAmount, state.scanlineSpacing, state.scanlineThickness, ctx.globalAlpha)
      }
      ctx.restore()
  ...
  }, [getAudioSnapshot, image, store.globalBackgroundEnabled, store.globalBackgroundUrl])

  if (!store.globalBackgroundEnabled || !store.globalBackgroundUrl || !image) return null
