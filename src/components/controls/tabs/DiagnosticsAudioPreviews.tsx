...
import { buildChannelWeightedCurve, downsampleFftMax } from '@/lib/audio/spectrumBinSampling'
import {
  getDebugBgAudio,
  getDebugLogoAudio,
...
export default function DiagnosticsAudioPreviews() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const bgWeighted = bgSnap && bins.length > 0
        ? buildChannelWeightedCurve(bins, BG_BAND_BARS, bgSnap.resolvedChannel)
        : []

      const spSnap = getDebugSpectrumPrimary()
      const spBars = spSnap && spSnap.barCount > 0 ? spSnap.barCount : 24
      const spWeighted = spSnap && bins.length > 0
        ? buildChannelWeightedCurve(bins, spBars, spSnap.resolvedChannel)
        : []

      const clSnap = getDebugSpectrumClone()
      const clBars = clSnap && clSnap.barCount > 0 ? clSnap.barCount : 24
      const clWeighted = clSnap && bins.length > 0
        ? buildChannelWeightedCurve(bins, clBars, clSnap.resolvedChannel)
        : []

      const lgSnap = getDebugLogoAudio()
      const lgWeighted = lgSnap && bins.length > 0
        ? buildChannelWeightedCurve(bins, LOGO_BAND_BARS, lgSnap.resolvedChannel)
        : []
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-[11px] leading-snug text-cyan-800">{t.hint_diagnostics_previews}</p>

      <div className="flex flex-col gap-1">
        <div className="flex items-baseline justify-between gap-2">
          <span className="text-[10px] font-semibold uppercase tracking-wide text-teal-500">{t.label_diag_bg}</span>
          <span className="text-[9px] text-gray-600">{t.label_diag_row_fft} / {t.label_diag_row_band}</span>
        </div>
        <canvas ref={canvasBg} className="h-13 w-full rounded border border-cyan-950/80" />
        <span ref={lineBg} className="block font-mono text-[10px] leading-tight text-gray-400" />
      </div>

      <div className="flex flex-col gap-1">
        <div className="flex items-baseline justify-between gap-2">
          <span className="text-[10px] font-semibold uppercase tracking-wide text-cyan-500">{t.label_diag_spectrum}</span>
          <span className="text-[9px] text-gray-600">{t.label_diag_row_fft} / {t.label_diag_row_band}</span>
        </div>
        <canvas ref={canvasSp} className="h-13 w-full rounded border border-cyan-950/80" />
        <span ref={lineSp} className="block font-mono text-[10px] leading-tight text-gray-400" />
      </div>

      <div className="flex flex-col gap-1">
        <div className="flex items-baseline justify-between gap-2">
          <span className="text-[10px] font-semibold uppercase tracking-wide text-violet-400">{t.label_diag_spectrum_clone}</span>
          <span className="text-[9px] text-gray-600">{t.label_diag_row_fft} / {t.label_diag_row_band}</span>
        </div>
        <canvas ref={canvasSpCl} className="h-13 w-full rounded border border-cyan-950/80" />
        <span ref={lineSpCl} className="block font-mono text-[10px] leading-tight text-gray-400" />
      </div>

      <div className="flex flex-col gap-1">
        <div className="flex items-baseline justify-between gap-2">
          <span className="text-[10px] font-semibold uppercase tracking-wide text-amber-400">{t.label_diag_logo}</span>
          <span className="text-[9px] text-gray-600">{t.label_diag_row_fft} / {t.label_diag_row_band}</span>
        </div>
        <canvas ref={canvasLg} className="h-13 w-full rounded border border-cyan-950/80" />
        <span ref={lineLg} className="block font-mono text-[10px] leading-tight text-gray-400" />
      </div>
    </div>
  )
}
