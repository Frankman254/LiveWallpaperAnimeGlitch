...
import { TRACK_TITLE_FONT_LABELS, TRACK_TITLE_FONTS, TRACK_TITLE_LAYOUT_LABELS, TRACK_TITLE_LAYOUTS } from './trackTitleOptions'

export default function TrackTitleTab({ onReset }: { onReset: () => void }) {
  const t = useT()
  const store = useWallpaperStore()
  const { captureMode, getFileName } = useAudioContext()
  const isFile = captureMode === 'file' && store.audioCaptureState === 'active'
