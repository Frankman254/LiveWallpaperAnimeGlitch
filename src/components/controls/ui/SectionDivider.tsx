...
export default function SectionDivider({ label }: Props) {
  ...
  return (
    <div className="flex items-center gap-2 my-1">
      <div className={`flex-1 h-px ${theme.panelSubtle.replace('text-', 'bg-')}`} />
      {label && <span className={`text-xs uppercase tracking-widest whitespace-nowrap ${theme.panelSubtle}`}>{label}</span>}
      <div className={`flex-1 h-px ${theme.panelSubtle.replace('text-', 'bg-')}`} />
    </div>
  )
}
