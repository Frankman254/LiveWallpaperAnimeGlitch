...
export default function PresetSelector() {
  ...
  function handleSave() {
    const currentName = customPresets[activePreset]?.name ?? activePresetName
  ...
  function handleDuplicate() {
    ...
    const name = window.prompt(t.prompt_duplicate_preset_name, suggestedName)
    if (name === null) return
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs ${isPresetDirty ? 'text-amber-400' : 'text-cyan-600'}`}>
          {activePresetName}
      ...
      {customPresetList.length > 0 && (
        <div className="flex flex-col gap-1">
          <span className="text-xs text-cyan-600 uppercase tracking-widest">{t.custom_presets}</span>
          <div className="flex gap-2 flex-wrap">
