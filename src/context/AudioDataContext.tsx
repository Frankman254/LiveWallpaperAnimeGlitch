...
export function AudioDataProvider({ children }: { children: ReactNode }) {
  ...
  const sourceIdRef = useRef(`audio-${Math.random().toString(36).slice(2)}-${Date.now()}`)
  ...
  const broadcastSnapshot = useCallback((snapshot: AudioSnapshot) => {
    const channel = channelRef.current
    if (!channel) return
    const now = typeof performance !== 'undefined' ? performance.now() : Date.now()
    if (now - lastBroadcastMsRef.current < AUDIO_SYNC_INTERVAL_MS) return
  ...
  const stopCapture = useCallback(function stopCapture() {
    ...
    resetAudioAnalysis()
    broadcastEmptyState()
  }, [broadcastEmptyState, resetAudioAnalysis, setAudioCaptureState, setAudioPaused])
  ...
  const getFileName = useCallback(function getFileName() {
    return analyzerRef.current?.getFileName?.() ?? remoteMetaRef.current.fileName ?? ''
  }, [])
  ...
  const getAudioSnapshot = useCallback(() => {
    ...
    if (!analyzerRef.current) {
      if (
        ...
        && (timestampMs - remoteSnapshotRef.current.timestampMs) < AUDIO_SYNC_STALE_MS
      ) {
