export const en = {
  ...
  hint_diagnostics_intro: 'Turn on HUDs for on-canvas readouts. Below: same analyser bins you hear, vs the band-weighted curve for each effect’s resolved channel (after auto-kick routing and channel smoothing). Spectrum gain envelope is shown as text.',
  hint_diagnostics_previews: 'Gray row: full FFT (downsampled max per bucket). Colored row: per-bar weights matching the circular spectrum for that layer’s resolved channel.',
  ...
  label_diag_waiting_spectrum: 'No spectrum snapshot yet (enable spectrum overlay).',
  label_diag_waiting_logo: 'No logo snapshot yet (enable logo overlay).',
  label_diag_clone_idle_preview: 'Clone not drawn this frame (linear + circular clone + logo).',
  // Actions
  reset_tab: '↺ Reset tab',
  ...
  hint_capture_window_audio: 'Opens the browser picker so you can capture a tab, window, or screen that includes audio.',
  ...
  hint_shared_bg_settings: 'Audio reactivity applies to the full loaded image set.',
  ...
  hint_shuffle_order: 'Reordering the pool changes the slideshow playback order.',
  confirm_shuffle_order: 'Shuffle the image order? You can cancel now if this was a misclick.',
  section_transition_next: 'Transition To Next',
  hint_transition_next: 'These transition settings are saved on the active image and are used when leaving it for the next one.',
  ...
  hint_restore_default_stack: 'Resets layer order to defaults and re-enables the main background layers.',
  ...
  hint_bg_scale_meter: 'HUD: base scale, audio boost as 0–10 vs max range, channel drive, and total scale (base + boost).',
  ...
  hint_bg_zoom_audio: 'Controls how the image scales with the beat. Pick a preset for a quick start, or expand below to tune attack, release, and the adaptive peak/floor.',
  section_background_motion: 'Background Motion',
  hint_background_motion: 'Parallax is the subtle mouse/device drift applied to the wallpaper stack. It belongs here because it changes BG movement, not audio analysis.',
  ...
  hint_editor_diag_tip: 'Live values on canvas: open the Diag tab and enable the matching HUD.',
  hint_editor_diag_spectrum: 'FFT, channel, and gain in real time: Diag tab (spectrum HUD + routing preview).',
  ...
  hint_spectrum_diag_hud: 'Live FFT routing, mean bin energy, envelope gain, and placement (incl. follow logo).',
  label_spectrum_diag_hud_title: 'Spectrum diagnostics',
  label_spectrum_diag_no_data: 'No spectrum frame yet (enable spectrum layer).',
  label_spectrum_diag_primary: 'Primary',
  label_spectrum_diag_clone: 'Circular clone',
  label_spectrum_diag_clone_idle: 'Clone not drawn (needs linear mode + circular clone + logo).',
  ...
  hint_logo_diag_hud: 'Live channel drive, envelope scale, and spectrum↔logo link.',
  ...
  hint_filter_target: 'Choose which image layer receives RGB shift, scanlines, film grain, and color filters.',
  // Fit mode
  ...
  hint_auto_channel_priority: 'Auto channel priority: Kick first. If kick drops below threshold and stays there for the hold time, the engine falls back to the loudest of Instrumental, Bass, HiHat, then Vocal.',
  ...
  hint_circular_clone: 'When the main spectrum is linear, also draw a radial copy attached to the logo if the logo is enabled.',
  section_circular_clone: 'Clone Settings',
  label_linear_direction: 'Linear Direction',
  label_fit_around_logo: 'Fit Around Logo',
  hint_fit_around_logo: 'Keep the inner edge clear around the logo while the outer contour keeps the selected radial shape.',
  ...
  hint_record_preview: 'For a clean export, capture this tab or the mini player window from the browser capture picker.',
  ...
  hint_mini_player_document_pip: 'Mini Player uses Document Picture-in-Picture when the browser supports it, keeping the wallpaper in a small always-on-top window.',
  hint_mini_player_popup: 'This browser falls back to a floating popup window. It works across more desktop browsers, but it is not the same as a true always-on-top mini player.',
  hint_mini_player_unavailable: 'Mini Player is unavailable in this browser. Fullscreen should still work when supported.',
  // MP3 upload
  ...
  hint_pause_all: 'Freezes slideshow transitions, particles, rain, and audio-driven motion while keeping the scene visible.',
  hint_pause_audio_only: 'Pauses the loaded track when available, or pauses audio reactivity while keeping visual motion running.',
