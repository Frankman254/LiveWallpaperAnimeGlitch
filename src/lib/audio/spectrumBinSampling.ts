...
export function getBandRange(mode: ResolvedAudioReactiveChannel, binsLength: number): [number, number] {
  switch (mode) {
    case 'kick':
      return [1, 14]
    case 'instrumental':
...
export function downsampleFftMax(bins: Uint8Array, bucketCount: number): number[] {
