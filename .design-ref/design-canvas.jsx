...
function DesignCanvas({ children, minScale, maxScale, style }) {
	...
	React.Children.forEach(children, sec => {
		...
		const hidden =
			persisted.srcKey === srcKey ? persisted.hidden || [] : [];
		const srcIds = [];
		abs.forEach(([aid, ab]) => {
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
	...
	React.useEffect(() => {
		...
		const zoomAt = (cx, cy, factor) => {
			...
			const next = Math.min(
				maxScale,
				Math.max(minScale, t.scale * factor)
			);
		...
		>
			<div style={{ padding: '0 60px' }}>
				<div className="dc-sectionhead" style={{ paddingBottom: 36 }}>
					...
					/>
					{subtitle && (
						<div style={{ fontSize: 16, color: DC.subtitle }}>
							{subtitle}
						</div>
					)}
				</div>
...
function DCArtboardFrame({
	...
	const doExport = kind => {
		...
		const name = String(label || id || 'artboard').replace(
			/[^\w\s.-]+/g,
			'_'
		);
	...
	return (
		<div
			style={{
				...
				padding: '14px 16px',
				fontFamily:
					'"Comic Sans MS", "Marker Felt", "Segoe Print", cursive',
				fontSize: 14,
				lineHeight: 1.4,
				color: DC.postitText,
				boxShadow:
