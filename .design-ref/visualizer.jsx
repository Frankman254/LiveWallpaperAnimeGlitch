...
window.VisualizerMock = function VisualizerMock({ mode = 'bars', accent = '#67e8f9', dim = false }) {
  ...
  useEffect(() => {
    ...
    const tick = (t) => {
      ...
      for (let i = 0; i < bins; i++) {
        ...
        target[i] = (lowFreq * 0.5 + midFreq * 0.3 + beat * 0.4) * fall * (0.7 + Math.random() * 0.3);
      ...
      // Background gradient
      const grad = ctx.createRadialGradient(W * 0.5, H * 0.6, 0, W * 0.5, H * 0.6, W * 0.7);
      ...
      ctx.arc(W * 0.5, H * 0.55, 80 * dpr + beat * 240 * dpr, 0, Math.PI * 2);
      ctx.fill();
      ...
      if (mode === 'bars') {
        ...
        for (let i = 0; i < bins; i++) {
          ...
          const x2 = Math.cos(a) * (R + v * R * 1.6), y2 = Math.sin(a) * (R + v * R * 1.6);
        ...
        for (let i = 0; i < bins; i++) {
          const x = (i / (bins - 1)) * W;
          const y = H * 0.55 + Math.sin(time * 2 + i * 0.2) * data[i] * H * 0.25;
      ...
      for (let i = 0; i < 30; i++) {
        ...
        ctx.beginPath();
        ctx.arc(px, py, (1 + (data[i % bins] || 0) * 3) * dpr, 0, Math.PI * 2);
        ctx.fill();
      }
